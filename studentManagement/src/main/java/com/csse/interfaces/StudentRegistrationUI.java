	...
	private void initialize() {
		...
		btnRegister.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				Student pstu = new PostGraduateStudent(stuID,fullNameTxt.getText(),nameWithInitTxt.getText(),nicTxt.getText(),
					...
					String jsonInString = mapper.writeValueAsString(pstu);
		...
		btnRegister.setBounds(266, 576, 89, 23);
				...
				int action = JOptionPane.showConfirmDialog(null,"Do you want to update this record?","Update",JOptionPane.YES_NO_OPTION);
					...
					if (StringUtil.isBlank(fullNameTxt.getText()) || StringUtil.isBlank(nameWithInitTxt.getText()) || StringUtil.isBlank(nicTxt.getText())
						...
						if (StudentValidation.validateNicNo(nicTxt.getText())) {
							...
							if (action == 0) {
							Student pstu = new PostGraduateStudent(fullNameTxt.getText(), nameWithInitTxt.getText(),
							...
							try {
								...
								HttpMessaheHandler.httpService("POST", jsonInString,
										"http://localhost:8081/admin.student/student/update");
							} catch (Exception e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
		...
		updateBtn.setBounds(25, 576, 89, 23);
		...
		deleteBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(action == 0) {
				try {
					//HttpMessaheHandler.httpService("POST",null,"http://localhost:8081/admin.student/student/update/'"+nic+"'");
					DAOManager dmger =new DAOManager();
					StudentDAO sdao = sdao = (StudentDAO) dmger.getDAO(DAOType.STUDENTDAO.dao());;
					Connection con = dmger.getConnection();
					sdao.deleteStudent(con,nic);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
				loadTable();
